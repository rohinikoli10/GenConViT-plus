...
def train_model(
    ...
    if test_model:
        test(model, cnn3d_model, dataloaders, dataset_sizes, mod, weight)


def test(model, cnn3d_model, dataloaders, dataset_sizes, mod, weight):
    print("\nRunning test...\n")
    model.eval()
    cnn3d_model.eval()
    ...
    for inputs, labels in dataloaders["test"]:
        ...
        cnn3d_features = cnn3d_model(inputs)  # Extract CNN3D features
        if mod == "ed":
            output = model(inputs, cnn3d_features).to(device).float()
        else:
            output = model(inputs, cnn3d_features)[0].to(device).float()
...
def gen_parser():
    ...
    parser.add_option("-c", "--cnn3d", dest="cnn3d", help="Use CNN3D model.")
    ...
    use_cnn3d = options.cnn3d

    return dir_path, mod, epoch, pretrained_model_filename, test_model, int(batch_size), use_cnn3d


def main():
    start_time = perf_counter()
    path, mod, epoch, pretrained_model_filename, test_model, batch_size, use_cnn3d = gen_parser()
    train_model(
        path,
        mod,
        epoch,
        ...
        test_model,
        batch_size,
        use_cnn3d=use_cnn3d,  # Pass use_cnn3d option to train_model function
    )
